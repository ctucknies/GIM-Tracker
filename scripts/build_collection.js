...
async function main() {
    ...
    const wikiData = JSON.parse(fs.readFileSync(path.join(DATA, 'clog_items_wiki.json'), 'utf-8'));
    ...
    const lootData = JSON.parse(fs.readFileSync(path.join(ROOT, 'loot_data.json'), 'utf-8'));
    ...
    if (fs.existsSync(path.join(ROOT, 'wiki_clog_obtained.json'))) {
        const wikiData2 = JSON.parse(fs.readFileSync(path.join(ROOT, 'wiki_clog_obtained.json'), 'utf-8'));
    ...
    fs.writeFileSync(path.join(DATA, 'collection_log.json'), JSON.stringify(result, null, 2));

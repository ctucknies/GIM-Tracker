...
async function handleRefreshClog(res, playerName) {
    try {
        ...
        const wikiItems = JSON.parse(fs.readFileSync(path.join(DATA, 'clog_items_wiki.json'), 'utf-8'));
        ...
        let lootData = [];
        if (playerName.toLowerCase() === 'drykeys' && fs.existsSync(path.join(ROOT, 'loot_data.json'))) {
            lootData = JSON.parse(fs.readFileSync(path.join(ROOT, 'loot_data.json'), 'utf-8'));
        }
        ...
        fs.writeFileSync(path.join(DATA, filename), JSON.stringify(result, null, 2));
